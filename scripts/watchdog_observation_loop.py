...
#
# ⚠ **표본 수를 문구에 박는다.** 「N건 중 M건」이라고 적어야 다음 표본이 나왔을 때 이 문구가
# 낡았다는 것이 보인다 — 「대체로」라고 쓰면 영원히 안 늙는다. 새 표본이 나오면
# `cliff_episodes.md`의 표와 **이 상수를 함께** 고칠 것.
#
# ===== 2026-09-16 (09-16 §3-13 / 제5부 고도화 1) — 낡은 것이 실제로 보였다 =====
#
# 위 문장이 예고한 일이 일어났다. 절벽 표본이 **여섯**이 됐고(08-14 · 08-26 · 09-01 · 09-03 ·
# 09-08 · 09-16), 회복이 관측된 **다섯 중 셋**만 15:25~15:26이다. 09-08이 15:30(마감 +10분)으로
# 처음 어긋났고 09-16이 15:29:51(+9분51초)로 **두 번째 연속** 어긋났다.
#
# ⚠ **다섯 번째(09-08)에 고쳤어야 했는데 못 고쳤다.** 그날 `cliff_episodes.md`는 이탈을 기록하면서도
# *"그 문구를 고치라는 뜻은 아니다 — 오늘도 인용은 정확했다"* 로 남겼다. 인용이 정확한 것과
# **사람이 그것을 보장으로 읽는 것**은 다른 문제이고, 두 번째 이탈이 그 차이를 확정했다.
#
# ⛔ **재기동 임계도 경보 조건도 여전히 안 건드린다.** 늘어난 것은 사람이 읽을 문장뿐이고,
# 억제(에피소드 첫 분 + 30분마다)는 한 줄도 안 바뀌었다 — 줄 **수**는 종전과 같다.
_NO_INGEST_RECOVERY_HINT = (
    "참고: 같은 유형(no_ingest)의 과거 표본 중 **회복이 관측된 5건에서 3건**"
    "(08-26 · 09-01 · 09-03)이 45~85분 만에 **15:25~15:26**(정규장 마감 +5~6분)에 자연 "
    "회복했다. **나머지 둘은 어긋났다** — 09-08 15:30(마감 +10분) · 09-16 15:29:51(+9분51초)로 "
    "최근 두 번 연속이다. 표본 여섯 모두 우리 쪽 조치는 없었다 — 자동 조치가 아니라 사람이 "
    "「지금 재기동할 것인가」를 판단할 근거이고, **보장이 아니라 과거 표본이다**"
)
