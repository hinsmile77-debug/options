...
# Absorption 캡션 — **판정 기준을 화면에 적는다.** 문턱이 종목마다 다른 값이 되므로(그 종목의
# 최근 변동성에서 나온다) 「가격 정체」라고만 쓰면 두 Radar가 같은 잣대인 줄로 읽힌다.
_ABSORPTION_CAPTION = (
    "「가격 정체」는 봉의 **범위(고가−저가)** 가 직전 20봉 범위 중앙값의 절반 이하일 때입니다"
    "(최소 2틱). 순변화가 아니라 범위를 보는 이유는 봉 안에서 크게 왕복했다가 제자리로 돌아온 "
    "봉을 정체로 세지 않기 위해서입니다. 문턱이 **종목의 최근 변동성에서 나오므로** 선물과 "
    "옵션이 같은 뜻을 갖습니다. 거래량 기준선도 같은 20봉이고, 최소 5봉이 쌓이기 전 구간은 "
    "0이 아니라 **판정 불가**로 표시됩니다."
)
