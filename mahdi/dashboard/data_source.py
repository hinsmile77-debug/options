...
def _watchdog_liveness_check(now: datetime) -> HealthCheck:
    ...
    **프로세스 생존은 기능 생존의 증거가 아니다.**
    ...
    # 2026-08-17 — **감시 창 안의 IDLE은 "감시를 껐다"는 뜻이다.**
    #
    # 여기까지 왔다는 것은 이미 `in_watch_window(now)`가 참이라는 뜻이므로, 이 IDLE은 「창 밖이라
    # 조용함」이 아니라 정지 표식이나 기동 표식으로 **판정을 보류 중**인 상태다. 종전 코드는 이
    # 갈래가 없어 아래 초록으로 떨어졌을 것이다 — "정상 — 5초 전 판정(idle)". 판정이 최신인 것과
    # 그 판정이 감시인 것은 다르다(바로 위 08-14 Fix#2와 같은 종류의 침묵이고, 이쪽이 더 나쁘다:
    # `degraded`는 적어도 되살릴 워치독이 살아 있지만 여기서는 **아무도 되살리지 않는다**).
    #
    # 기동 중(수십 초~300초)도 같은 갈래로 들어온다. 그것은 곧 풀리는 상태라 warning이 과하지만,
    # 갈래를 나누느니 detail을 그대로 인쇄해 사람이 읽게 한다 — 배지를 조용히 초록으로 두는 것보다
    # 잠깐 노란 것이 낫다는 판단이다(규약 D의 오경보/미탐지 비대칭이 여기엔 적용되지 않는다.
    # 이 배지는 자동 조치를 유발하지 않는다).
    if action == liveness.ACTION_IDLE:
        return HealthCheck(
            label, "warning",
            f"워치독이 감시 창 안에서 **판정을 보류 중** — {check.get('detail') or 'IDLE'} · "
            "이 구간에 관측 루프가 죽어도 아무도 되살리지 않는다",
            group="인프라",
        )
