...
먼슬리("regular")/위클리(월, "weekly_mon")/위클리(목, "weekly_thu") 세 북의 ATM±2 유동성
스냅샷을 나란히 비교한다(2026-07-10 위클리를 요일별 두 상품으로 분리) — 장전 선발 스코어러
(Phase 2, 아직 미구현)가 20거래일 기준선으로 쓸 원자료를 사람이 먼저 눈으로 확인할 수 있게
한다. 근거: docs/Dev_md/RESEARCH_EXPIRY_SELECTION_v1.md.
"""
...
_SERIES_LABEL_KO = {"regular": "먼슬리", "weekly_mon": "위클리(월)", "weekly_thu": "위클리(목)"}

_MONTHLY_EXPIRY_WEEK_NOTE = (
    "※ 이번 주는 먼슬리 만기 주 — 목요일 위클리(위클리(목)) 신규 상장이 없어 먼슬리가 그 역할을"
    " 대신합니다(위클리(목) 행은 차주 위클리를 관측 중 — 위클리(월) 행은 영향 없음)."
)
...
def _is_monthly_expiry_week(rows: list[dict], today: date) -> bool:
    ...
    계산: "regular" 북의 만기가 today와 같은 (연도, ISO주차)에 속하면 이번 주가 먼슬리
         만기 주라고 판단한다. KRX는 먼슬리 만기 주의 목요일에는 위클리(목)을 별도 상장하지
         않으므로(2026-07-10 확인), 이 경우 "weekly_thu" 행은 자동으로 차주 위클리를 가리키게
         된다(위클리(월)은 요일이 달라 영향 없음) — 사용자가 데이터 누락(대시)으로 오인하지
         않도록 명시적으로 알려준다.
    """
    for r in rows:
