...
# ===== 2026-09-03 — **여전히 안 켠다. 그리고 08-11의 「켤 조건」을 폐기한다** =====
#
# 09-03에 14:31~15:23 **53분 연속 전멸**이 났다(08-11의 22분·08-14의 84분에 이은 세 번째).
# 사용자가 COCKPIT 감마맵의 지수 현재가가 브로커와 15p 벌어진 것을 보고 시작된 점검이다 —
# 그 15p의 본체가 이 전멸이었다(지수 스팟은 옵션 시세 응답에 얹혀 오므로 체인이 0행이면 0행이다).
#
# ## ① 재검토 조건은 **충족되지 않았다** — 08-14와 같은 서명이다
#
# 위 재검토 조건은 「`inquire-balance` 기준 오후 지연이 6.0초 미만인 전멸일」이다. 09-03의
# 미검열 통로(300초 창, 창당 1콜이므로 p50 = 그 호출의 실측 지연):
#
#     14:25  3.34   14:30  2.53  ← 마지막 정상
#     14:35 10.06   14:40 10.03   14:45 10.03   14:50 10.03
#     14:55 10.06   15:00 10.01   15:05 **12.44**   15:10 10.02
#     15:15 10.06   15:20 10.03   15:25  7.14  ← 회복
#
# **전멸 53분 내내 KIS 실제 지연이 10초 이상**이었다(자기 천장 10.0초에 눌리고 12.44는 넘겼다).
# 6.0초로 올려도 이 호출들은 전부 그대로 실패한다 — 08-16이 08-14 데이터로 내린 판정이
# 다른 날 다른 표본에서 그대로 재현됐다.
#
# ## ② 「켤 조건」(08-11)은 충족됐다. 그런데 켜면 안 되는 날이었다 — 그래서 폐기한다
#
#     조건 앞절  timeout_abort.count > 0        09-03: **117건** ✓
#     조건 뒷절  budget_exceeded < 08-11(87건)   09-03: **85건** ✓   (08-14는 138건이라 깨졌다)
#
# 조건의 의도는 *"Fix#1이 실제로 조기에 접었다는 증거"* 였다. 그런데 09-03이 그 조건을 통과한
# 이유는 **날이 너무 나빠서 예산이 마르기도 전에 전부 타임아웃으로 접혔기** 때문이다(abort 117회).
# 즉 이 조건은 「Fix#1이 건강하게 작동」과 「사고가 예산 단계까지 가지도 못함」을 **같은 숫자로
# 낸다.** 겨눈 것의 반대 방향에도 켜지는 조건은 발동 조건이 아니다.
#
# **폐기한다** — 이 레버의 발동 조건은 이제 위 「재검토 조건」 하나뿐이다. 숫자를 보고 조건을
# 고치는 것이 아니다(그건 규약이 금한다): 조건이 **충족된 날에 그 조건대로 켰다면 손해**였다는
# 반증이 나왔으므로 조건 자체를 내리는 것이고, 그 반증을 여기 남긴다.
#
# ## ③ 08-16이 레버 **대신** 채택한 조기경보가 양방향으로 실패했다 — 이게 더 크다
#
# 08-16은 이 레버를 접으면서 그 자리에 *"천장 없는 통로의 절대 지연"* 을 선행지표로 놓았다
# (08-14에 12:31 4.19 → 13:31 8.16 → 14:06 10.02로 절벽을 95분 앞서 올라갔다는 근거).
#
#   위음성(09-03)  14:25 3.34 → 14:30 **2.53** → 14:35 10.06. **램프가 없다.** 절벽 5분 전
#                  값이 그날 오후 최저 수준이었다 — 95분 예고가 이날은 **0분**이다.
#   위양성(09-02)  13:20 **6.19** · 14:25 **6.12** 로 6.0 임계를 두 번 넘겼는데 전멸 **0분**,
#                  스팟 380/380분 완전 적재. 임계를 넘긴 날이 오히려 완벽한 날이었다.
#
# 즉 「6.0초 임계」는 예고력이 없다. **대체 지표를 여기서 정하지 않는다** — 08-16이 08-14
# 하루로 정했다가 이렇게 뒤집힌 것과 같은 실수가 된다. 표본을 더 쌓고 별건으로 설계한다.
#
# ## ④ 검열 모델의 정정 — max는 천장을 넘는다
#
# 08-16 주석은 4.0을 넘는 호출이 *"전부 4.0~4.06으로 기록된다"* 고 적었다. **p50에는 맞지만
# 분포 전체에는 틀리다**: 09-03 전멸 중 `inquire-price`의 max가 6.33 / 6.01 / 6.47 / **6.89**
# 초였다. httpx의 read 타임아웃은 총 응답시간이 아니라 **소켓 read 사이의 간격**에 걸리는 값이라,
# 느리지만 끊기지 않고 오는 응답은 천장을 넘겨서 완료된다. 이 통로를 근거로 쓸 때 「p50은
# 눌린 값 / max는 안 눌린 값」임을 구분할 것.
#
# ## 귀속 — 우리 쪽이 아니다 (다음에 같은 로그를 볼 사람을 위해)
#
#   · **WS는 무사했다**: 전멸 53분 동안 선물 1분봉 **53/53분**(거래량 12,921). 로컬 회선이
#     아니라 KIS **REST 게이트웨이** 국소 장애다. 이 대조가 귀속을 가르는 가장 싼 방법이다.
#   · **그날의 변수는 설명하지 않는다**: 09-03은 Fix#10(`SIGNAL_FUSION_PHASE_OFFSET_SECONDS`
#     10→55초)만 움직인 날인데 그것은 **DB 전용 판단 위상**이고 REST를 안 쓴다
#     (로그의 `타폴러동시호출추정=0건`). 세 엔드포인트(balance·asking·price)가 **동시에**
#     무너진 것도 KIS 귀속을 가리킨다.
#
# ## 그날 우리가 할 수 있었던 것 — 없다. 그래서 화면을 고쳤다
#
# 53분 내내 매분 20레그가 **전부** 실패했다. 스팟은 레그 하나만 성공해도 나오는데(14:30은
# `rows=1`인데 스팟이 적재됐다) 그 하나도 없었다. 전용 지수 경로를 뒀어도 balance·asking이
# 같이 죽은 날에 살아남았을 근거가 없다. 그래서 09-03에 실제로 고친 것은 폴링이 아니라
# **못 받은 값을 현재가인 척 그리지 않는 것**이다(`dashboard.data_source.spot_is_stale`).
