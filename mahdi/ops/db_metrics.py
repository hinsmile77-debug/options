...
_DAILY_TABLES: list[tuple[str, str, str]] = [
    ...
    # 2026-08-19 (08-18 보고서 §3-2 / Fix#2) — **성공한 왕복이 어떤 지표에도 안 잡혔다.**
    #
    # 08-18 14:03 실주문 왕복(제출→조회→취소) 네 단계가 전부 `rt_cd=0`으로 성립했는데,
    # 그 가설(`2026-08-16-c1`)의 주장 지표인 `db.tables`가 **이 테이블을 애초에 세지 않았다.**
    # 자동 판정은 그것을 「실측 없음」이 아니라 **「수기 판정」**으로 흘려보내 눈에 띄지 않았다.
    #
    # README가 경고한 「지표 경로가 실재하지 않아 가설이 조용히 무력화되는」 형태인데, **오타가
    # 아닌 모양**으로 나타났다 — 경로는 맞는데 그 경로가 세는 목록에 대상이 없었다. 그래서
    # 여기 한 줄을 더하는 것과 `scripts/measure_order_roundtrip.py`가 실제로 적재하는 것을
    # **함께** 해야 한다: 하나만 고치면 여전히 0행이거나 여전히 안 보인다.
    #
    # 다른 항목과 성격이 하나 다르다 — **매일 자라지 않는다.** 사람이 주문을 낸 날에만 늘어나므로
    # `rows = 0`은 결함이 아니라 「그날 주문이 없었다」이고, 그 구분은 사람이 한다(규약 C).
    ("execution_logs", "timestamp", "주문 실행 이력(사람이 주문한 날만 자란다)"),
]
