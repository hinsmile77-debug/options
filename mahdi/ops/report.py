...
def _render_hypotheses(results: list[dict], db: dict | None = None) -> list[str]:
    ...
    # 2026-09-16 고도화 2 (근거: 09-16 §3-12) — **배너를 안 읽고 표만 훑는 사람을 위한 열.**
    # 09-03부터 표 **위**에 절벽 경고가 떠 있었는데, 09-16에 그 경고가 정확히 맞는 날
    # (`gex_input_missing_minutes` 86분 · 기준선 3분)에도 표 안에는 아무 표시가 없었다.
    # ⛔ **새 임계를 만들지 않았다** — 조건은 위 배너와 **같은 것 하나**다. 같은 사실을 두 축으로
    #    세면 어느 쪽이 맞는지 묻는 날이 온다.
    # ⛔ **`verdict`는 한 글자도 안 바뀐다.** 자동으로 판정을 무르는 것이야말로 「숫자를 보고
    #    기준을 고치는 것」이고, 이 열이 하는 일은 판정 **옆에 사실 하나를 적는 것**뿐이다.
    # ⚠ **이 열은 그날 전체가 절벽일인가만 말한다.** 어느 행이 실제로 오염됐는지는 지표별 측정
    #    창을 알아야 갈리는데 그 정보가 여기 없다 — 모르는 것을 아는 척하지 않는다.
    # ⚠ **절벽이 없는 날의 출력은 한 글자도 안 바뀐다** — 그날은 열 자체가 안 실린다.
    cliff_day = cliff_minutes >= threshold
    head = ["id", "가설", "지표", "실측", "예측", "역할", "판정"]
    rows = [
        [
    ...
    if cliff_day:
        out.append(
            "> **오염**: `⚠`는 **오늘이 절벽일**이라는 뜻이고 그 행이 반드시 오염됐다는 뜻은 "
            "아니다 — 어느 지표가 실제로 그 구간에 걸쳤는지는 사람이 가른다. 위 배너와 **같은 "
            "사실**을 표 안에서 한 번 더 말하는 것이고, 절벽이 없는 날은 이 열이 아예 없다 "
            "(2026-09-16 고도화 2)."
        )
    out.append("")
    return out
