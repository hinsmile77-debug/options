...
2. **DB를 읽지 않는다.** 일별 지표(`docs/동작점검/auto/YYYY-MM-DD_지표.json`)가 커밋되는
   위치에 매일 쌓이므로 그것을 접기만 한다. 새 폴러·테이블·스케줄러가 없다.

   ⚠ **08-18 이 모듈이 생긴 시점에는 이 문장이 사실이 아니었다.** `.gitignore`가
   `docs/동작점검/auto/`를 통째로 무시하고 있었고(2026-08-01 결정 — 그때는 이 json의 수명이
   하루라 로컬에만 둬도 잃을 게 없었다), 그래서 이 모듈은 **백업이 없는 파일 위에 표본을
   쌓고 있었다.** 로그는 이틀치만 남아 재생성도 안 되므로 PC가 죽으면 캠페인이 통째로 사라진다.
   08-19에 `.gitignore`를 고쳐 `_지표.json`만 추적으로 되돌리고 이 문장을 사실로 만들었다 —
   문서를 사실에 맞추는 대신 사실을 문서에 맞춘 것이고, 이유는 **코드가 요구하는 쪽이
   옳았기 때문**이다. `min_days: 10`을 쓰는 순간 이 파일은 중간 산물이 아니라 관측이다.
