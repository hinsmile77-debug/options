...
_QUALITATIVE_ALWAYS_PRESENT = (
    ...
    # 2026-09-17 제4부 Fix C — **전멸이 0건인 날에도 키가 실려야** 한다. 이 값이 §1 「한눈에」의
    # 판단 입력 표에 행으로 올라가는데(`report.HEADLINE_JUDGEMENT_LOG_METRICS`), 키가 없으면
    # 그 칸이 `—`로 찍히고 **`—`는 「재지 않았다」이지 「좋았다」가 아니다** — 08-14에 §1이
    # 완벽한 하루를 인쇄한 것이 정확히 그 오독이었다. 전멸은 안 나는 날이 흔하므로 이 등록이
    # 없으면 그 행은 대부분의 날에 빈칸이 된다.
    "chain_cycle_empty",
)
