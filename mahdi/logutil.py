...
class TracebackBudget:
    ...
    def describe(self) -> str:
        """기동 로그에 남길 한 줄 — **카운터가 프로세스 단위라는 사실을 로그가 말하게 한다.**

        2026-08-06(§3-3 / Fix#4): 그날 프로세스가 세 번 떠서 `오늘 4번째`·`오늘 12~15번째`가
        로그에 **두 번씩** 나왔다(중복 7건). 카운터는 위 docstring대로 "프로세스 수명 = 하루"를
        전제하는데 그날 그 전제가 깨졌고, **깨졌다는 사실이 로그 어디에도 없었다.** 사후에
        `오늘 N번째`를 하루 누계로 읽으면 실제(205건)를 190건으로 과소평가한다.
        """
        return (
            f"트레이스백 예산 초기화 — 이 프로세스에서 예외 유형당 {self._samples}건까지 "
            "트레이스백을 남긴다('오늘 N번째'는 하루가 아니라 이 프로세스 기준이다)"
        )
