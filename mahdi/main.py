...
#
# ===== 2026-08-26 (08-26 §1-8 / P1-3) — **채우는 시각은 「다음 거래일 장전」이다** =====
#
# 이 dict를 채우는 규칙(`hypotheses.yaml`의 `2026-08-04-p5-kis-latency-rule`)에 **발동 시점이
# 비어 있었다.** 그래서 규칙은 08-04에 만들어진 뒤 3주째, **네 번 성립하고 네 번 다 발동하지
# 않았다** — 미룬 것이 아니라 **결정을 내릴 수 있는 시각이 하루에 한 순간도 없었다**:
# 조건은 「이틀 연속 같은 시간대」이고 그것을 아는 시각은 장후 지표(15:46)인데, 그때 그
# 시간대는 **언제나 과거**다.
#
# **채운다면 07:30 기동 전이다.** 이 값은 프로세스 기동 시점에 읽히고, 장중에 채우려면
# 재기동이 필요하다 — 그 대가는 **봉 30개(약 30분)**다.
# ⚠ **「예열 89분」이 아니다**(08-26 §1-19 정정). 그 숫자는 기동 시각의 함수다: 08-26은
# 07:44 기동 → 89분, 08-25는 07:30 기동 → 103분인데 **둘 다 09:15:00에 끝났다.** 앞의 62분은
# 장전이라 봉이 아예 안 만들어진 구간이다. 두 회차가 「89분」을 재기동 배제의 첫 근거로 썼고,
...
# 같은 형식으로 적혀 있다.
#
# ===== 2026-09-15 아침 — **9시를 추가로 켰다.** 09-09·10·11·14 나흘 연속(비거래일 09-12·13
# 제외) 미결 끝에 사용자가 (A)를 선택(2026-09-14 장후 점검 「사용자 조치」 1번). 사전 대응
# 규칙(`2026-08-04-p5-kis-latency-rule`)의 9시대 성립이 09-14 기준 20구간까지 누적된 것이
# 근거. 대가: 9시대 위클리 커버리지 약 50%→약 25% 반토막(09-14 장후 §종합 판정표 인용).
# 07:39 시점 프로세스가 이미 07:30 기동으로 옛 값을 읽은 채 떠 있어, 반영을 위해 재기동한다
# (장중이 아니라 개장 전이라 재기동 대가는 적용되지 않는다 — 규약: 재기동 금지는 09:00 이후).
OPTION_CHAIN_SLOW_SERIES_CONGESTED_HOURS: dict[int, int] = {9: 4, 10: 4, 11: 4, 12: 4, 13: 4, 14: 4}
...
#
# ===== 2026-09-02 — **켠다. 다만 25.0이 아니다** (09-02 장후, 사용자 승인) =====
#
# 위 08-11 블록은 3주 넘게 그대로 있었다. 값은 오늘까지 10.0이었고, 레버 상태표의 「켜짐」은
# *상수가 존재한다*는 뜻이었지 Fix#10이 적용됐다는 뜻이 아니었다. 미룬 사유(같은 날 Fix#1과
# 겹치면 귀속이 안 갈린다)는 08-11에는 옳았고, 그 뒤로는 **아무도 다시 안 봤다.**
#
# ## 25.0을 버리는 이유 — 시스템이 그새 무거워졌다
#
# 08-11 근거는 «20레그 최악 19.3초»였다. 오늘 `observation_loop.log`에서 **사이클이 분 안의
# 몇 초에 끝나는가**(`옵션체인 사이클 소요 분해` 줄의 타임스탬프)를 7거래일 전수 집계했다 —
# 혼잡 10~14시, 매일 300사이클:
#
...
#
# **25초에 판단하면 7일 전부 100% stale이다.** 08-11의 예측치를 그대로 켰다면 지표는 한 칸도
# 안 움직였을 것이고, 우리는 그것을 「Fix#10이 실패했다」로 읽었을 것이다. 값이 낡은 것을
# 못 본 채로. 사이클이 REST 예산(50초, Fix#8)에 눌려 있으므로 **끝나는 시각의 천장은 구조적으로
# 약 57초**다 — 09-02 실측 최댓값도 55.6초(전 시간대)였다.
#
# ## 왜 55.0인가 — **오늘 고른 값이 아니다**
#
# 55.0은 `docs/동작점검/2026-08-19_마흐디_운영점검보고서.md` §5 고도화 4 (a)가 **숫자를 보기
# 전에 적어 둔 값**이다. 위 표는 그 값을 고른 근거가 아니라 **검산**이다(그것이 이 저장소의
# 순서다 — 숫자를 보고 예측을 고치지 않는다). 57.0이 7일 모두 100%를 주지만 08-28 최댓값
# 56.7초와 0.3초 차이라 여유가 없고, 시스템은 08-11 이후 실제로 무거워졌다.
#
# 08-19가 (b) 홀짝 A/B를 권고하면서 *"레버 E 결정 이후에 착수"* 라고 단 조건은 **09-02에
# 충족됐다**(레버 E 판정 종결). A/B는 「신선한 체인이 더 잘 맞히는가」(`g5`)를 위한 것이고,
# 이 상수는 그 전에 **`current` 표본 자체를 만든다** — 08-19 §7의 「남은 질문 1」이 그것이었다.
#
# ## 예측치 (숫자를 보기 전에 적는다 — `hypotheses.yaml: 2026-09-02-fix10-phase-offset-55s`)
#
#   주장  `db.signal_reach.chain_input_source.stale_pct` <= 20.0 (09-02 실측 99.2%)
#         ⚠ 이 지표는 레버 E에 대해서는 2단계 간접이었지만(규약 J / §1-11), **이 상수에
#         대해서는 직접이다** — 판정식이 `newest의 분 == now의 분`이고 이 상수가 `now`를 정한다.
#   주장  `db.signal_reach.chain_newest_age_seconds.p50` <= 55.0 (마이그레이션 036, 오늘 신설)
#   대가  판단 시각이 45초 늦어진다 — ADVISORY라 무해하지만 실주문 배선 시에는 진입 지연이다.
...
def _build_signal_inputs(
    ...
    # 2026-09-02(09-02 §1-11 / 제4부 P1-5) — 바로 위 이산 판정의 **연속판**. 마이그레이션 036.
    #
    # 08-11에는 이진값으로 충분했다(하루의 절반이 stale이었으므로 절반이 갈렸다). 그런데
    # 08-18 이후 이 값은 **매일 98~100% stale**이다 — 판정이 「분 단위 완전 일치」라 1초만
    # 어긋나도 stale이고, **1분 늦은 체인과 3분 늙은 체인이 같은 칸**에 들어간다. 그래서
    # 09-02에 레버 E의 「주장」 축으로 `stale_pct`를 읽었을 때 «레버가 실패한 것»과 «지표가
    # 애초에 그 축을 못 재는 것»을 가를 수 없었다(§1-11).
    #
    # **같은 `newest`에서 낸다** — 두 값은 서로를 검산한다:
    #   `chain_newest_leg_age_seconds < 60`  ⟺  `chain_input_source == "current"`
    # (레그 타임스탬프는 분 단위로 잘려 있으므로 나이 = `now.second` + 마이크로초다 —
    #  즉 이 값은 판단 초보다 **작지 않고 같다**. 60초를 넘는 순간이 분이 갈리는 순간이다.)
    # 정의를 바꾸는 것이 아니라 **나란히 내는** 이유는 회귀다: `stale_pct`의 정의를 손대면
    # 08-11 이후 전 이력과의 비교가 끊긴다(P1-5의 회귀 위험 항목).
    ...
    chain_inputs = {
        ...
        # 2026-09-02 P1-5 — 마이그레이션 036. 위가 스냅샷 창의 **너비**라면 이것은 그 창의
        # **최신 끝**이다. 위상 레버(Fix#10)가 움직이는 것이 바로 이 값이다.
        "chain_newest_leg_age_seconds": chain_newest_leg_age_seconds,
