...
- [ ] **`_record_http_latency()`가 국내/해외 `inquire-price`를 한 행에 섞는다** (P2) —
      ...
      안전하므로, 계측 쪽만 같은 방식으로 맞추면 된다.
- [ ] **장 개시 전 `underlying_spot_1m` stale 스팟** (P2) — 08-05 08:45/08:46에 스팟이
      1000.03으로 남아 있었다(실제 1048.50). 08:46 ATM 롤링으로 해소되지만, 그 두 분의 VRP·GEX는
      전부 그 스팟 기준이다. Fix#1 범위 밖으로 남겼다.
