...
- [ ] **Slack 알림 활성화 여부** — `slack_alert_settings.enabled`가 2026-07-28부터 false다.
      KRX 서킷브레이커 감지 시 보내도록 만든 `CRITICAL` 알림이 현재 어디에도 도달하지 않는다.
...
- [ ] **폴러별 전용 페이서 분리 — 보류 중, 재검토 조건 있음** — 지금은 KIS 앱키 전체가 단일 공유
      `_RateLimiter`(1건/초)를 쓴다(2026-07-08에 페이서를 쪼갰다가 500 폭주로 203분치 데이터 유실을
      겪은 뒤 의도적으로 합침). 스태거링만으로 스케줄 밀림이 충분히 안 줄어드는 게 하루치 데이터로
      확인되면 그때 재검토 — 단, 먼저 KIS 실제 한도가 앱키당 총합인지 TR ID/엔드포인트별인지부터
      확인(공식 문서 또는 별도 저부하 테스트)하지 않고는 라이브에 바로 적용하지 말 것
      ([[DECISION_LOG]] 참고).
