...
## 2026-07-31 Fix 6종 반영 후 다음 거래일 검증 항목 (07-30 운영점검 후속)

- [ ] **Fix#2/#3 효과 확인** — 다음 거래일 하루치로 (a) 시간대별 REST수집 평균이 07시→14시로
      단조 증가하지 않는지(07-30: 32.0→58.2초), (b) 분(mod 5)별 타폴러동시호출 평균이 특정 분에만
      몰리지 않는지(07-30: 분%5=3만 22.9건, 나머지 1.7~6.0건), (c) 옵션체인 사이클 유실이
      25분(5.1%)에서 줄었는지 확인. 집계 쿼리는 보고서 §2-1 표와 동일 방법.
- [ ] **Fix#1 효과 확인** — `signal_decisions`에서 ENTER가 419건 → 0건(또는 실제 진입 셀에서만
      발생)으로 바뀌고 `reject_reason='strategy_palette:wait_only'`가 정상적으로 쌓이는지.
- [ ] **Fix#4 최초 관측** — `observation_loop.log`에 "장운영정보(H0UNMKO0) 수신 N건" INFO가
      실제로 남는지가 **CB 감지 생존 여부의 최초 증거**다. 남으면 감지기 정상, 하루 종일 0건이면
      구독/파싱이 깨진 것이므로 그때 원인을 좁힌다. 수신 빈도를 확인한 뒤 "N분 이상 미수신 = 경고"
      임계를 정할 것(지금은 근거 없는 오경보를 피하려고 일부러 임계를 안 넣었다).
- [ ] **Fix#6 확인** — `risk_snapshots`가 매 분 쌓이는지(07-30엔 0행). 읽기 함수를 추가할 때
      `loss_buffer`(DECIMAL → Decimal) `float()` 변환 잊지 말 것.
- [ ] **Fix#5 자연 해소 시점(2026-08-04 예상)** — 정규장 기준 유효 종가가 18개 → 21개가 되는
      날부터 `feature_store.rv_ratio`가 1.0이 아닌 값을 갖기 시작해야 한다. 실제로 바뀌는지 확인.
