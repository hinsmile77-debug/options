...
- [ ] **HMM 학습 임계 도달 예상 2026-08-08(금)** — `feature_store` 6,623건(08-04) / 82.8%,
      일 410건 페이스. 07-31 예상(08-11)보다 사흘 이르다.
      **08-07(목) 저녁 드라이런 필수** — `--dry-run`으로 (a) 잠재상태 방문 수가 8/8인지
      (b) `rv_ratio` 분산이 0을 벗어났는지 확인한 **뒤에** 저장할 것.
      `fit()`이 예외를 안 던진다는 것은 성공 기준이 아니다.
      **주의**: 앙상블 가용 멤버가 2개인 상태에서 레짐이 다양해져도 판단은 안 바뀐다 —
      Fix#2·#4의 효과(멤버 4개)를 먼저 확인한 뒤에 레짐 학습 결과를 평가할 것.
- [ ] **`base_size` 하드코딩(1.0)·단위 미정의, 재진입 방지 로직 부재** — 2026-07-29 기록,
      실거래 전 필수, **7일째 미해결**.
