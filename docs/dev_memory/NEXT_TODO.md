...
- [ ] **리플레이로 검증된 것(재확인만)**: 슬롯 고갈 없음(3북 활성 최대 39/41),
      Fix#1 창 자르기(실DB 327분), Fix#3(순수 함수), HMM 저장 가드(드라이런).
- [ ] ⚠ **리플레이로 못 잡는 것 — KIS WS가 「유지 구독」에 계속 데이터를 보내는가.**
...
- [ ] **기동 로그에 `retention_pool` 경로가 실제로 탔는지** 확인 — 매니저 3개 생성 블록은
      단위테스트가 안 타는 유일한 경로다(`main()` 안). 07:31 첫 롤에서 해제 로그가
      **안 나오는 것**이 정상 신호다.
- [ ] 마이그레이션 028은 08-07 장중에 이미 적용해 뒀다 — 장전 재적용은 멱등이라 무해하다.
