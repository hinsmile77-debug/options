...
- [ ] **총 REST 수요 축소안 선택** — 현재 0.663건/초로 공유 페이서 용량의 66%. 백오프 1.51배부터
      구조적 적자라 스태거링만으로는 한계다. (a) 위클리 2북 격분화 → 0.50건/초(**권장** — 먼슬리
      1분 해상도 유지, 잃는 게 가장 적음) (b) ATM±2→±1 → 0.47건/초(GEX/Vanna 신호 폭 축소)
      (c) 만기유동성 300→600초 → 0.62건/초. 전부 수집 데이터 범위를 바꾸는 제품 결정이라
      코드만 보고 임의 진행하지 않는다.
