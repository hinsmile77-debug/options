...
- [x] (2026-07-10 유력 해소, 요일 확정만 남음) **위클리(목) 확인**: 2026-07-06 당시 상장 전이라
      미확인이었던 "위클리(목)" 체인 — 2026-07-10 마스터파일 재확인 결과 상품종류 `L`(콜)/`M`(풋)에
      "위클리C/P 2607W3" 행이 실제로 존재함을 확인, `symbol_master.py`의 `_SERIES_PRODUCT_TYPES`에
      추가 완료([[DECISION_LOG]] 2026-07-10 항목 참고). N/O("위클리M ", 2607W2)와 L/M("위클리",
      2607W3)이 동시에 상장돼 있어 최소 "두 코드풀 교대 배정"은 확정됐으나, L/M이 정말 "위클리(목)"이고
      N/O가 "위클리(월)"인지(또는 요일과 무관한 단순 교대인지)는 이름만으로 확정 불가 —
  - [ ] 실거래 중 `get_quote()`의 `futs_last_tr_date`로 N/O·L/M 각 풀의 실제 만기 요일(월/목)을
        확인해 `symbol_master.py` 주석에 확정 기록.
