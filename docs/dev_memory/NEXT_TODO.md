...
- [ ] **`event_proximity_minutes`가 여전히 None** (08-05 Fix#3에서 의도적으로 남김, P2) —
      **사용자 결정 대기**. `classify()`는 이 값이 15분 미만이면 ×0.5 페널티를 거는데,
      지금은 FOMC·옵션만기·고용지표 직전에도 페널티가 안 걸린다. 캘린더 소스가 없어 못 채운다.
      값을 지어내면 무작위 페널티가 되므로 **소스를 먼저 정해야 한다**:
  - (a) **수기 YAML 테이블**(`docs/동작점검/event_calendar.yaml` 등) — 외부 의존 0,
        구현 작음. 대가는 **사람이 매달 채워야 하고, 빠뜨리면 조용히 페널티가 안 걸린다**
        (지금과 같은 상태로 되돌아가는데 그것이 안 보인다).
  - (b) **만기일만 우선 배선** — `option_analysis_1m.expiry`로 이미 알 수 있어 추가 소스가
        0이다. 다만 v6 §11.2가 말하는 "매크로 이벤트"의 일부만 덮으므로 **커버리지 착시**
        위험이 있다(FOMC는 여전히 안 걸리는데 "이벤트 근접도 배선됨"으로 보인다).
  - (c) **외부 캘린더 API** — 커버리지는 가장 좋지만 yfinance 폴백과 같은 운영 신뢰도 문제를
        하나 더 만든다([[DECISION_LOG]] 2026-07-20 "실거래 전환 시 데이터 소스 재검토" 참고).
  - 어느 쪽이든 **"소스가 비어 있음"과 "이벤트가 없음"을 구분**해야 한다 — 그 구분이 없으면
        08-05에 찾은 결함들과 같은 형태가 된다.
