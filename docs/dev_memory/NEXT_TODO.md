...
- [ ] **Docker Desktop 기동 후 라이브 DB 직접 대조** — 2026-07-22 세션 내내 Docker가 꺼져있어
      `option_analysis_1m.theta`의 실제 컬럼 타입(`\d option_analysis_1m`)과 `shutdown_check_log`
      최근 기록을 라이브로 확인 못 함(로그 근거로는 정상, §2-2). Docker 뜨면 1회 대조 권장.
