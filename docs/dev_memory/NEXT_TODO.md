...
## 2026-08-04 장마감 후 — **GEX 광폭 체인 결정** (사용자: "장마감 후 결정")

### 왜 결정이 필요한가 (08-04 07:44 실측으로 확정)

감마플립 산출률 0%의 원인은 **코드가 아니라 행사가 창**이다. 오늘 체인은 완벽하다
(`iv=0` 0건, `gamma=0` 0건, 만기 전부 미래). 그런데 각 북 안에서 콜−풋 OI 불균형의 부호가
한 방향으로만 쏠려 있어 GEX(S)가 ±5% 전 구간에서 부호를 안 바꾼다:

```
만기 08-06  콜OI−풋OI {982.5:-4, 985:+1744, 987.5:+2504, 990:+1758, 992.5:+204}  → 전 구간 +54~64B
만기 08-13  콜OI−풋OI {982.5:-208, 985:-124, 987.5:-14, 990:-465, 992.5:-23}     → 전 구간 -5.0~-5.5B
만기 08-10  OI 전부 0 (08-03 롤오버된 신규 북)
```

ATM±2(5행사가)로는 부호가 뒤집힐 자리가 없다. 그리고 `options_flow` 멤버는 `gamma_flip`이
None이면 절대 안 붙으므로 **`2026-08-03-p1`의 `member_count_max >= 3` 예측도 여기에 달려 있다.**

### 결정 규칙 — **숫자를 보기 전에 적어둔다**

오늘 자동 리포트에서 다음 4개를 읽는다:

| 지표 | 경로 |
|---|---|
| 초당 수요 | `rest.calls_per_second` |
| 용량 대비 | `rest.capacity_pct` |
| 적자 시작 배율 | `rest.deficit_threshold_multiplier` |
| 백오프 최대 | `backoff.max_multiplier` |

**추가 가능한 분당 콜 수** = (목표 초당수요 − 오늘 실측 초당수요) × 60

목표 초당수요는 아래 **두 조건을 모두** 만족하는 값으로 잡는다:
- (A) `capacity_pct <= 55` — 07-30에 66%에서 25사이클이 통째로 유실된 전례가 있다.
- (B) **추가 후 적자 임계 > 오늘 백오프 최대** — 임계 아래로 내려가면 구조적 적자다.
  적자 임계 ≈ 1 / 목표 초당수요.

후보(먼슬리 전용 ATM±7 = 30레그):

| 주기 | 분당 추가 | 초당 수요(8/3 기준) | 적자 임계 | 감마플립 해상도 |
|---|---|---|---|---|
| 10분 1회 | +3 | 0.501 (50%) | 2.00배 | 10분 |
| 5분 1회 | +6 | 0.551 (55%) | 1.81배 | 5분 |

> 8/3 백오프 최대가 **2.76배**였으므로 그 값이 오늘도 그대로면 **둘 다 (B)를 만족하지 못한다.**
> 그 경우 선택지는 (i) 더 긴 주기(15~20분) (ii) 더 좁은 폭(ATM±5) (iii) 보류 셋뿐이다.
> **Fix#2(위클리 짝/홀 분산)가 백오프 최대를 얼마나 낮췄는지가 이 결정의 전부다.**

- [ ] 위 4개 지표를 읽고 (A)(B)로 후보를 거른 뒤, 남는 것 중 가장 짧은 주기를 고른다.
- [ ] 고른 안을 `hypotheses.yaml`에 예측치와 함께 등재한다(감마플립 산출률 목표치 포함).
- [ ] 남는 게 없으면 **보류하고 그 사실을 기록**한다 — 예산이 안 되는데 넣으면 08-03처럼
      밀림·결손이 판단 입력을 망친다.

## ⚠ 오늘(08-04) 관측 루프에는 08-03 저녁 이후 커밋이 반영돼 있지 않다

프로세스가 07:30에 떴으므로 **오늘 수집 데이터는 `4a8ea4f` 기준**이다.
- `ce05b2d`(COCKPIT 배지 2건), `3d51fbf`(b) 만기유동성 패널 — **표시 계층이라 데이터 무관**,
  COCKPIT 프로세스만 재시작하면 즉시 반영된다.
- `3d51fbf`(a) 구독 ACK 즉시 DB 기록 — **`main.py` 변경이라 관측 루프 재시작 필요**.
  장중 재시작은 데이터 결손을 만드므로 **내일 장전 기동부터** 적용된다.
  → 오늘 아침 CB 배지 오경보(07:31~07:36)는 내일 사라진다.
