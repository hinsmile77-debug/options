...
- [x] **(2026-07-31 결정·구현 완료) 총 REST 수요 축소안 — 사용자가 (a)+(c) 조합 승인**.
  ...
  - [ ] **다음 거래일 검증**: 초당 수요가 실제로 0.45 근처인지(로그 HTTP Request 수 ÷ 관측 분),
...
- [ ] **ZN/ES 실패 로그 축약 여부** — 계좌 거래소(CBOT/CME) 미신청이라 매일 99×2건 100% 실패가
      확정이고, 매번 풀 트레이스백이 남아 `observation_loop.log`가 하루 12.8MB로 커진다(장중
      11:52 회전 발생). 다만 신청이 언제 열릴지 모르는 상태라 진단 정보를 성급히 줄이지 않는 편이
      나을 수 있어 보류 중.
