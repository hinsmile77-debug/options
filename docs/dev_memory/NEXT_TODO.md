...
- [ ] **내일 아침 `premarket_startup.log`에서 "기동 전 잔존 프로세스 정리" 블록이 실제로
      찍히는지 확인** — 오늘 신규 추가한 로직(§1-1 대응)의 첫 실전 검증. 평소엔 잔존 프로세스가
      없어 "없음"으로 조용히 통과하는 게 정상이고, 이 로그 줄 자체가 안 보이면 배치파일이
      제대로 갱신 안 된 것.
- [x] **레이트리미터 회복 임계값(8) 재조정 후 며칠 더 실측(2026-07-24 완료·정정)** — 07-23
  ...
  - [ ] **다음 거래일(2026-07-27 월) 확인 필요**: 20으로 원복한 뒤 EGW00201 비율·스케줄 밀림이
        07-22 수준(0.38%, 57건)으로 돌아오는지, 신규 추가한 "레이트리밋 백오프 확대/회복"
        로그가 실제로 찍히는지(지금까지 단위테스트로만 검증) 확인.
  - [ ] **COCKPIT "레이트리밋 근접도" 배지(2026-07-24 신규, 10번째 배지) 브라우저 렌더링 확인**
        ...
        확인 필요(지금까지는 단위테스트만).
  - [ ] **`db/migrations/014_rate_limiter_status.sql` 라이브 적용 확인** — 다음 장전 기동 시
        `premarket_startup.log`에 "적용 완료 - 014_rate_limiter_status.sql"이 정상 출력되는지.
- [ ] **ZN(10년 국채선물) KIS·yfinance 이중 실패 13:01 패턴 재확인(2026-07-23 발견)** — 07-22
      13:01:29, 07-23 13:01:24로 이틀 연속 5초 오차 내로 재현됨(운영점검보고서 2026-07-23 §2-3).
      다음 거래일에도 13:01 전후로 재현되는지 로그로 확인 — 3일 연속이면 우연이 아니라 KIS 또는
      yfinance 쪽 정기 요인(유지보수 윈도우 등)으로 확정, 아니면 2회는 우연으로 결론.
- [ ] **COCKPIT 기동 마커(`logs/.last_cockpit_start.txt`) 실전 확인** — 내일부터 COCKPIT이 정상
      기동될 때 cockpit.log에 "직전 COCKPIT 기동: ..." 줄이 실제로 남는지, 좀비 프로세스 판별에
      쓸모 있는 형태로 찍히는지 확인.
