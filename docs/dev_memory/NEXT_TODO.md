...
- [ ] **(08-05) 가용 멤버 2 → 4 확인.** 오전 3 / 오후(14:00~) 4가 정상 모양이다
      (Charm 경로는 v6 §13.2대로 14:00부터 열린다). 리포트 **§14-1**이 멤버별로 분해해 보여준다.
- [ ] **(08-05~06) `options_flow`가 유의미한 신호를 내는지** — 감마월 폴백이 방향을 뒤집어
...
- [ ] **(~2026-08-08 금) `feature_store` 8,000샘플 도달** — 08-04 6,623건(82.8%), 일 410건 페이스.
      07-31 예상(08-11)보다 사흘 이르다.
- [ ] **(08-07 목 저녁) 드라이런 필수** — `--dry-run`으로 (a) 잠재상태 방문 수가 8/8인지
      (b) `rv_ratio` 분산이 0을 벗어났는지 확인한 **뒤에** 저장할 것.
      `fit()`이 예외를 안 던진다는 것은 성공 기준이 아니다(08-03 드라이런에서 비수렴 결함 2건을
      그렇게 잡았다).
- [ ] 성공 기준은 **`regime_state`에 RANGE_BALANCED 외 값이 나오는 것**(현재 21영업일 100% 고정).
      **주의**: 축 A가 2개인 상태에서 이 축만 개선되면 판단은 거의 안 바뀐다 — 08-05에 멤버 4개를
      먼저 확인하고 나서 레짐 학습 결과를 평가할 것. **순서를 바꾸면 원인 귀속이 불가능해진다.**

### 축 C — 실행 (판단이 살아난 **뒤에** 필요한 것)

- [ ] **`base_size` 하드코딩(1.0)·단위 미정의** — 2026-07-29 기록, **7일째 미해결**.
