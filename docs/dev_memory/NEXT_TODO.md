...
- [ ] **`RATE_LIMITER_STATUS_WRITE_SLOW_SECONDS`(0.2초) 경고가 실제로 찍히는지, 스케줄 밀림
      건수가 07-24(115건) 대비 개선되는지 확인** — 07-24 운영점검보고서 §2-1/§4 Fix#1로 계측만
      추가하고 제어 흐름(asyncio.to_thread 등)은 바꾸지 않았다. 이 경고가 스케줄 밀림 시각과
      겹쳐서 찍히면 이 동기 DB 쓰기가 실제 원인이라는 근거가 되고, 안 찍히거나 겹치지 않으면
      다른 원인(예: main.py 곳곳의 기존 블로킹 DB 쓰기 패턴 전반, 5-2 고도화 방안 참고)을
      찾아야 한다 — 지금까지는 단위테스트로만 검증(실제 REST/DB 소요시간은 미실측).
- [ ] **ZN 이중 실패 Slack 알림이 연속 2회(10분) 조건으로 정상 동작하는지 확인** — 07-24
      §4 Fix#3로 `ZN_DUAL_FAILURE_ALERT_STREAK=2` 도입. 13:01 패턴이 4일째도 재현되면 이번엔
      알림이 13:06(두 번째 연속 실패 사이클) 근처에 뜨는지, 여전히 단일 사이클 블립이었던
      적은 없었는지(즉 스트릭 조건이 실제로 뭔가를 걸러낸 적이 있는지) 로그로 확인 필요.
- [ ] **COCKPIT 레이트리밋 근접도 배지 브라우저 픽셀 렌더링 확인(사용자 확인 필요)** — 07-23 §4
      Fix#4부터 이어지는 항목. 2026-07-24 저녁 세션에서 Docker+COCKPIT을 직접 띄워 검증을
      시도했으나, 이 세션(Claude Code CLI)에는 브라우저/스크린샷 도구가 없어 육안 렌더링 자체는
      확인 불가 — 대신 (1) `data_source.get_health_summary()`를 라이브 DB에 직접 호출해 10번째
      배지가 정확히 "ok | 15:43:00 기준 배율 1.00배, 직전 사이클 밀림 0.0초" 형태로 나오는 것
      확인, (2) COCKPIT(Streamlit) 프로세스를 실제로 기동해 에러 없이 뜨고 `curl`로 HTTP 200
      확인(스크립트 자체 실행은 WebSocket 연결이 있어야 트리거되므로 이걸로는 미검증) — 이
      두 가지로 배지 로직·데이터 배선은 확정됐지만, Streamlit이 이 값을 실제로 어떤 색상/배지
      모양으로 그리는지는 사용자가 브라우저로 한 번 열어 확인해줘야 함.
...
- [x] **레이트리미터 회복 임계값(8) 재조정 후 며칠 더 실측(2026-07-24 완료·정정)** — 07-23
  ...
  - [x] **다음 거래일(07-24) 확인 완료**: 20으로 원복한 뒤 EGW00201 비율이 정확히 07-22 수준
        ...
        회복 280건, 최대 배율 2.25배).
  - [ ] **COCKPIT "레이트리밋 근접도" 배지(2026-07-24 신규, 10번째 배지) 브라우저 렌더링 확인**
        — `data_source._rate_limiter_health_check()` 추가 후 COCKPIT을 재시작해 배지가 실제로
        보이는지, 장중 배율이 1.0을 넘는 순간을 실측으로 한 번 이상 포착해 warning으로 바뀌는지
        확인 필요(07-24 cockpit.log에 관련 에러는 없어 코드 레벨 정상 추정이나 육안 미확인).
