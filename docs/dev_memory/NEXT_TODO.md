...
## 2026-08-04(화) 검증 항목 — 08-03 구현한 Fix 6종 + 고도화 6종

> 예측치는 `docs/동작점검/hypotheses.yaml`의 `2026-08-03-p1~p5`에 등재돼 있고 08-04 자동 리포트
> §0이 대조한다. 아래는 **자동 대조로 안 잡히는 것**과 육안 확인 항목만 적는다.

- [ ] **`hypotheses.yaml`의 `상태`를 손으로 확정할 것**(2026-08-03-p1~p5). 08-01 6건은 확정 완료.
      리포트 §0 상단에 "확정 대기 N건" 콜아웃이 뜨면 그게 미확정 신호다(08-03 §5-4로 신설).
- [ ] **감마플립이 처음으로 값을 갖는지** — 자동 리포트 §14 `감마플립 산출률`.
      0%가 유지되면 코드가 아니라 **행사가 창(ATM±2)** 문제다 → 아래 "사용자 결정 대기 #1"로 승격.
- [ ] **`ATM 롤링` 로그가 장중에 실제로 찍히는지**(`mahdi.main`, 스팟 이동 시에만).
      하루 종일 0건이면 재롤링이 안 도는 것이다 — 08-03에 하루치 체인이 5.4% 외가격이었던 그 결함.
- [ ] **`피처 활성화: rv_ratio가 중립값(1.0)을 처음 벗어났다`** 로그 한 줄이 나오는지.
      **줄이 안 나오면 그 자체가 이상 신호**다(유효 종가 21일째 도달 예정일).
- [ ] **`WS 구독 확립: H0UNMKO0`** 로그와 `ws_status.market_op_subscribed_at` 값.
      장운영정보 **데이터가 0건인 날에도** 이 값은 있어야 한다 — 그게 이 fix의 전부다.
- [ ] 마이그레이션 **021/022**가 장전 기동 스크립트로 자동 재적용되는지(이미 라이브 수동 적용됨).
- [ ] `premarket_startup.log`의 리포트 생성 줄에 **한글 경로가 안 깨지는지**(§3-1 인코딩 fix).
- [ ] 자동 리포트 §1에 **전일 델타 열**이 처음으로 붙는지(08-03 사이드카가 생겼으므로).
- [x] **(2026-08-03 완료) COCKPIT 육안 확인** — 07-06부터 4주 이월된 항목. 장 마감 직후
      (15:44:53) 화면을 열어 **배지 버그 2건**을 잡았다(보고서 §3-2):
      먼슬리 커버리지 120.7%(분자/분모 기간 불일치가 초록불로 숨어 있었다), 선물 시세 배지가
      종가 단일가 구간(15:35~15:45)을 결손으로 봐 매 거래일 오경보. 둘 다 같은 세션에 수정.
- [ ] **08-04 재확인(브라우저)** — 08-03 화면은 07:30에 뜬 Streamlit 프로세스라 그날 저녁 코드
      변경이 반영돼 있지 않았다. **장전 기동으로 프로세스가 새로 뜬 뒤** 아래를 눈으로 볼 것:
  - [ ] "관측 품질"에 **신호 도달률** 배지(6번째)가 나오는지 — 08-03 데이터로는
        `⚠ 감마플립 0.0% · 앙상블 최대 2멤버`가 떴다. 08-04에 이 값이 바뀌어야 정상이다.
  - [ ] CB 배지에 **"구독확립 HH:MM:SS"** 가 나오는지. 안 나오면 `H0UNMKO0 구독 미성립` 경고가
        떠야 한다(둘 다 아니면 마이그레이션 021 미적용).
  - [ ] 먼슬리 커버리지가 **100%를 넘지 않는지**(넘으면 "기간 불일치" 경고가 뜬다).
  - [ ] 15:40~15:45 사이에 **선물 시세 배지가 초록으로 유지**되는지(오경보 재발 여부).
        단, 그 시간대에 화면을 열어야 확인된다.

## 이월 — 일정 대기

- [ ] **HMM 학습(2026-08-10경)** — `feature_store` 6,213건(08-03) → 8,000건.
      **드라이런은 08-03에 완료**했고 거기서 비수렴 결함 2건을 잡았다(§4-0b/c). 임계 도달일에는
      `--dry-run`을 **한 번 더** 돌려 (a) 잠재상태 방문 수가 여전히 8/8인지 (b) `rv_ratio` 분산이
      0을 벗어났는지 확인한 **뒤에** 저장할 것. `fit()`이 예외를 안 던진다는 것은 성공 기준이 아니다.
