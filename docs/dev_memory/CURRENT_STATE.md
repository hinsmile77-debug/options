...
_최종 갱신: **2026-09-22** (장후 자동조치 회차). 아래 절은 **날짜가 최신인 것이
우선한다** — 맨 위 절이 그 아래 모든 절보다 우선한다._

---

## [MW0601] 2026-09-22 — **조용한 날. 고친 것은 사건이 아니라 「이미 있는 줄 몰랐던 것」이다**

> 근거: `docs/동작점검/2026-09-22_마흐디_일일점검.md`(전 국면 + 제6부) ·
> `DECISION_LOG.md` 같은 날짜 절.

### 한 문장

P0 0건 · 거래 0건의 조용한 날이었고, 장후 자동조치가 **제4부 P2-2를 구현하지 않았다** —
그 기능은 **2026-08-26에 이미 들어와 있었고 오늘 사이드카가 그것을 세고 있었다.**
대신 「오늘 정하면 언제 반영되는가」를 달력으로 세게 했고, 그 과정에서 리포트의
**「항상 D+1」이 틀렸다**는 것이 드러났다(09-23의 반영일은 **09-28**이다).

### 오늘 하루

- **P0 0건 · P1 3건**(1-1 사전 대응 규칙 미결 · 1-2 가설 4건 · 2-3 데이터 공백) ·
  **P2 5건**. 신규 4건(1-5 · 1-6 · 2-3 · 2-4).
- **실주문 0건 · 체결 0건 · 보유 0건 · 실현손익 0원 · 절대원칙 위반 「판정 대상 없음」** —
  `ADVISORY` 유지. 진입 판단 **331건이 전부 차단**됐다
  (`mode_not_auto_submit` · `reentry_cooldown_not_configured`, 310회).
- **데이터 공백 1회 — 13:34~13:38(5분)**, 단발로 종결. 원인은 오전부터 누적된 **KIS 응답
  지연**이고 단일 트리거가 없다(09시 p50/timeout 0.41 → 12시 0.63 → 13시 0.99).
  백오프 최대 **1.82배**(전일 2.25보다 낮다)인데 타임아웃 449건 — 08-04 §2-6 규약대로
  **KIS 귀속**이다.
- **응답시간 압력 상시화**: `p95/timeout ≥ 1.0` 창 **48/98개**, 최장 연속 6창(12:11~12:36).
  ⚠ 「위험선 최초 돌파 없음」은 **안전하다는 뜻이 아니다** — 그 분의 p50이 검열 때문에 하한값이다.
- **종료 시퀀스 정상**(15:45:03 정시 · 잔존 프로세스 0 · 사이드카 15:47 생성).
- **재기동 0회 · 크래시 0건.** 09-21에 고친 `^C` 파서가 오늘 정상 기동을 제대로 읽었다
  (`crash.marker_present=True` · `starts=1`).

### 오늘 바뀐 것 (장후 자동조치)

1. **`market_calendar.next_trading_day()` 신설** — `previous_trading_day()`의 거울상.
   사이드카에 **`preemptive_decision`** 절이 생겼다(`effective_date` ·
   `effective_in_calendar_days`). 사전 대응 규칙 결정의 **반영일이 이제 산출물에 남는다.**
   🔴 **「항상 D+1」은 틀렸다** — 09-23의 반영일은 **09-28**(추석 09-24·25 + 주말).
2. **`phases.md` A-2 명문화** — 발동 창(07:30 전)과 장전 회차(08:00)의 **구조적 미스매치**.
   「오늘 아침이 발동 창」은 **어제 정해 둔 것을 누르는 창**이지 오늘 정하는 창이 아니다.
3. **증거 §5-2에 비영 구간 시작·종료 시각** — 리포트가 「특정 못 함」으로 닫은
   「비영 0 최장 74분」의 정체가 **07:31~08:46, 전부 장전**으로 드러났다.
   구조적 사유이지 신호 고갈이 아니다.
4. **가설 `2026-09-21-p1-1` → `confirmed`**(192 → 194항목 · pending 6 · confirmed 125).

### 🔴 안 바뀐 것 — 그리고 그것이 옳다

- **제4부 P2-2(토글과 무관한 결손 로그)는 이미 있다** — `main.py:3094` → `notify.py:122·157`
  → `log_metrics.py:503`, 오늘 값 `qualitative.alert_suppressed_toggle_off = **2**`.
  **함정 게이트가 걸러낸 아홉 번째.** 남는 차이는 임계(300→180초) 하나이고 **C등급**이다.
- **사전 대응 규칙 (A)/(B)는 25거래일째 미결.** ⚠ **09-23에 안 정하면 다음 기회는 09-28이다.**
- **ADVISORY 유지** — 오늘도 진입 판단 전건 차단. 설계된 설정이지 오류가 아니다.
  ⚠ `2026-08-19-fix2`가 영원히 검증 안 되는 이유이기도 하다.

### 테스트 · 커밋

**2,398 passed / 0 failed**(신규 15건 · 기존 무수정 · 그중 판정 무변경 축 2건).
`a756529`(산출물) · `b585fb0`(가설·예측치·축) · `b1fef95`(명문화·고도화 2).
...
## 2026-09-21(월) 기준 — 하루 한 파일 체제의 **첫 완전한 하루 사이클**

- **오늘 하루**: 거래 **0건** · 포지션 **0건**(ADVISORY 모드 + 진입요건 미충족) · 실현손익 0원.
  절대원칙 위반 **0건**. 관측 루프 07:30~15:45 **공백 0건**, 워치독 OK 49 / 비-OK 0,
  WS 단절 0건, `no_ingest` 절벽 **0회**(09-18의 9회·196분 다음 거래일에 완전 소강).
  체인 신선도 `stale` **0.6%** — 위상 레버(Fix#10) 도입 이후 가장 낮은 값 중 하나다.
- **보고서 체제**: 하루 한 파일(`{날짜}_마흐디_일일점검.md`)에 **4개 회차**(장전 08:02 ·
  장중① 12:35 · 장중② 14:41 · 장후 15:47)가 전부 정상 append 됐다 —
  **전환(08-20 개정) 이후 처음으로 완전한 하루 사이클을 확인**했다.
  여기에 장후 자동조치가 **제6부**를 덧붙여 닫는다(197행 추가 · **0행 삭제**).
- **오늘 고친 것**: 기동 표식(`===== 관측 루프 기동 =====`) 판정이 콘솔이 남긴 `^C` 접두에
  깨지던 것을 **두 파서에서 함께** 고쳤다 — `collect_evidence.py::_crash_marker()` ·
  `mahdi/ops/crash_metrics.py::_start_marker()`. ⚠ **리포트는 한 곳만 지목했고, 두 번째
  현장은 사이드카의 날짜별 시계열이 찾아 줬다**(`crash.starts` 1·1·1·1 → **0**).
- **테스트**: 전체 **2,383 passed · 0 failed**(신규 7건, 기존 스위트 무수정).
- **가설 파일**: **192항목** · pending **5** · confirmed 124 · untested 6 · inconclusive 48 ·
  refuted 8 · superseded 1. 09-18부터 3거래일 멈춰 있던 확정 작업을 **오늘 둘 닫아 끊었다**
  (`2026-09-17-fixc`→confirmed · `2026-09-18-p1-2`→untested). **넷은 여전히 사람 몫이다.**
- **아직 안 된 것 / 열려 있는 것**:
  - 사전 대응 규칙 레버 (A)/(B) 결정 — **24거래일째**. 오늘 **15시가 처음으로 조건에
    들어왔는데**(15:00~15:20 `inquire-price` p95 **3.05초**) 지금 레버(`mahdi/main.py:282`,
    `{9,10,11,12,13,14}`)는 **그 시간대를 아예 모른다.**
  - `.gitignore:52`가 `_지표.md`·`_증거_*.md`·`_지연창.tsv`를 거른다(**열사흘째**) —
    그래서 커밋되는 자동 산출물은 `_지표.json` 하나뿐이다.
  - `.claude/` `.gitignore` 편입 여부 **26일째** 미결정 · ruff `.venv` 미설치 **열세 회차 연속**.
  - 이 예약 작업의 완결 판정 가드가 **아흐레째** 예외로 통과 중이다(템플릿에서 완결 표기가
    09-09부터 빠졌다) — ⚠ 상시화 금지, 사람이 정할 것.
