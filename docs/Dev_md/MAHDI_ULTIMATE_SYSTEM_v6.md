...
| CVD | 공격적 매수−매도 체결량의 누적(**거래소 누적 분류값** `SHNU_CNTG_SMTN`/`SELN_CNTG_SMTN`의 차분, 못 읽으면 틱 룰 추정으로 폴백) | 기울기가 방향, 부호 전환이 주도권 교체. 화면 계열의 원점은 **조회 창의 시작**이라 절대 높이는 창에 종속된다 |
| Queue Imbalance | 최우선 호가잔량 비대칭 | 체결 압력 방향 |
| Absorption | 대량 체결에도 가격 불변 — 「불변」은 **봉 범위(고가−저가)가 그 종목 직전 20봉 범위 중앙값의 절반 이하**(최소 2틱) | 반전 or 지속의 핵심 단서. 문턱이 종목 자신의 변동성에서 나오므로 선물·옵션이 같은 뜻을 갖는다 |
...
- **Volume Spike / Exhaustion**: `spike = current_vol / avg_vol`, 3배 이상 + 가격 정체 = Absorption 의심
  - 「가격 정체」는 **고정 상수로 정의하지 않는다** — 순변화율 고정 문턱(0.05%)도 고정 틱 수도 선물·옵션 중 한쪽에서 상시 참 또는 상시 거짓이 된다(2026-08-21 실측: 고정 4틱 게이트의 통과율 선물 0.0% / 옵션 17.6%). 그 종목 **직전 20봉 범위의 중앙값 대비**로 잰다.
  - 순변화가 아니라 **범위**를 본다 — 봉 안에서 왕복했다 제자리로 온 봉은 흡수가 아니다.
...
## 11.5 Instrument Selection — 전략명 → 실제 종목·행사가 결정 (2026-08-17 신설)

> §11.4까지의 출력은 **전략 이름**(`atm_long`, `debit_spread`, …)이고, Execution의 입력은
> **단축상품번호**(`EntryContext.symbol`)다. 이 절 이전까지 그 사이를 잇는 계층이 스펙에
> 없었다 — 백테스트는 `symbol="BACKTEST"` 자리표시자로 그 빈칸을 메우고 있었다.

### 원칙

1. **후보 생성은 모드와 무관하게 항상 자동이다.** 하이브리드 3모드(§13.1)가 가르는 것은
   "그 선택을 실행할 것인가"이지 "선택을 누가 할 것인가"가 아니다. 사람이 매번 행사가를
   고르면 ADVISORY→CONFIRM 승격일에 검증된 적 없는 로직이 처음 돌게 된다(만들어 두는 것과
   도는 것은 다르다 — 워치독의 교훈).
2. **모의투자(ADVISORY) 단계부터 구성·가동한다.** 제안 종목을 `signal_decisions`에 기록만
   하고, 사람이 실제 고른 종목과의 **갈림(divergence)** 을 축적한다. 갈림의 감소가
   CONFIRM 승격의 정량 근거다.
3. **선택 근거를 함께 기록한다** — 왜 이 행사가인가(델타·OI·스프레드·만기 잔존일).
   계산해 놓고 버리면 "무엇이 판단을 밀었나"에 답할 수 없다(`member_scores`의 교훈).
4. **만기(북) 선택은 규칙이 아니라 실측**(`option_analysis_1m`에 관측된 만기)을 따른다 —
   공휴일 이월(2026-08-18 화요일 만기)은 요일 규칙으로 낼 수 없다. 만기 당일 북은 0DTE
   플레이북 전용이며 일반 전략의 후보에서 제외한다.

### 전략별 선택 파라미터 (초기값 — 모의투자 실측으로 보정)

| 전략 | 다리 | 행사가 규칙 | 만기 |
|------|------|------------|------|
| atm_long | 콜 또는 풋 1 | ATM 최근접 행사가 (선물 체결가 기준) | 최근접 비만기일 북 |
| itm_debit | 1 | \|델타\| 0.60~0.70 | 〃 |
| debit_spread | 2 | 매수 ATM ±1 / 매도 2~3행사가 밖 | 〃 (양다리 동일 만기) |
| atm_straddle | 콜+풋 2 | 동일 ATM 행사가 | 〃 |
| small_strangle_buy | 콜+풋 2 | 양쪽 \|델타\| ≈ 0.25 | 〃 |
| straddle_accumulate | 콜+풋 2 | 동일 ATM, 분할 진입(회당 상한은 Risk가 결정) | 〃 |
| limited_premium_sell 계열 | 1~2 | 게이트(§11.4) 통과 시에만 후보 생성 | 〃 |

### 유동성 하한 필터 (후보 제외 조건 — §1.3 "실행 품질 우선"의 구현 지점)

- `oi` 하한 미달, `bid_ask_spread` 상한 초과, 당일 체결 없음 중 **하나라도** 해당하면 후보에서
  제외하고 제외 사유를 기록한다. 임계값은 상수 하드코딩 금지 — `strategy_params.yaml`에 두되,
  **초기값은 모의투자 실측 분포를 본 뒤 정한다**(모르는 채 임계를 정하면 임계가 곧 결론이 된다).
- 전 후보가 필터에 걸려 빈 목록이면 그것은 "선택 실패"가 아니라 **판정**이다 —
  `no_liquid_instrument` 사유로 기록하고 진입하지 않는다(빈 목록의 사유 코드 규약과 동일).

### 승격 경로

```text
① 선택기 가동 + 기록      (모의투자 ADVISORY — 제안만, 주문 없음)
② 사람 선택과 갈림 측정    (며칠치 축적, hypotheses.yaml에 예측 선등재)
