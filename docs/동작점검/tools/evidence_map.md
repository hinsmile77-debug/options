...
| `docs/동작점검/{D}_점검_pre.md` | 장전 점검 세션 | 개장 전 판단 — 레버·가설·기동 |
| `docs/동작점검/{D}_점검_intra.md` | 장중 점검 세션 | 지금 살아 있는가 + 장전 지적사항의 현재 상태 |
| `docs/동작점검/{D}_마흐디_운영점검보고서.md` | 장후 점검 세션 / **사람** | 그날의 해석. 위 두 파일을 §1·§2로 흡수한다 |
| `docs/동작점검/hypotheses.yaml` | **사람(fix 구현 시점)** | 예측치. 다음 거래일 리포트가 자동 대조 |

> **폴더 규칙 — `auto/`는 기계가 만든 재료, 루트는 사람이 읽는 보고서다.**
> 지표·증거 다이제스트는 `auto/`에 쌓이고(하루 여러 개·회전 대상), 국면별 점검 보고서 셋
> (`_점검_pre` / `_점검_intra` / `_마흐디_운영점검보고서`)은 `docs/동작점검/` 루트에 나란히 둔다.
> 2026-08-13 이전의 `_점검_pre`는 `auto/` 아래 있었다 — 그 날짜 이전 파일을 찾을 때는 두 곳을 본다.
