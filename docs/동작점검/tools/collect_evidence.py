...
# 그 회차들이 읽은 것은 08-18 보고서의 체크박스 **원문**이었고, 폐기 사실은 `NEXT_TODO.md:72`와
...
# 그 항목은 **2026-08-01 사용자 결정으로 보류 확정**됐고 `NEXT_TODO.md:1210`이
...
#
# 1. **Fix 번호로 맞추면 틀린다.** `NEXT_TODO:72`의 「보고서 Fix#1을 폐기한다」는 **08-18**
...
def build(root: Path, day: _date, phase: str, cfg_phases) -> str:
    ...
    A("> 보류 확정된 Slack 토글을 다시 올렸다 — `NEXT_TODO:1210`이 *「매 점검 보고서에서 다시")
