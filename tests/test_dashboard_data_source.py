    ...
    responses = {
        **_BASE_RESPONSES,
        ...
        "chain": [
            ...
            # 032로 붙은 컬럼이다.
            (1340.0, "C", 363, 0.9, 0.0047, 1000.0, date(2026, 7, 9), ts, 0.72, 0.51, 120, 1, 7.25, "regular"),
            (1340.0, "P", 200, 0.85, 0.0040, -800.0, date(2026, 7, 9), ts, 0.72, -0.49, 90, 1, 6.80, "regular"),
        ],
...
def test_load_snapshot_gamma_map_uses_only_the_monthly_book_like_the_engine(monkeypatch):
    ...
    responses = {
        **_BASE_RESPONSES,
        "regime": [(ts, 2, [0.1] * 8, None, False, False)],
        "chain": [
            (1045.0, "C", 100.0, 0.60, 0.02, 1_000.0, monthly, ts, 0.5, 0.5, 10, 1, 7.25, "regular"),
            (1047.5, "P", 120.0, 0.62, 0.03, -400.0, monthly, ts, 0.5, -0.5, 10, 1, 7.25, "regular"),
            # 위클리 두 북 — 화면에 섞이면 안 된다(만기 Pinning은 별도 신호).
            (1045.0, "C", 900.0, 0.90, 0.09, 90_000.0, weekly_thu, ts, 0.5, 0.5, 10, 1, 7.25, "weekly_thu"),
            (1050.0, "C", 800.0, 0.85, 0.08, 80_000.0, weekly_mon, ts, 0.5, 0.5, 10, 1, 7.25, "weekly_mon"),
        ],
    }
...
def test_load_snapshot_reports_a_single_gamma_wall_matching_the_engine(monkeypatch):
    ...
    responses = {
        **_BASE_RESPONSES,
        ...
        "spot": [(1045.0, ts)],
        "chain": [
            (1040.0, "C", 10.0, 0.60, 0.01, 100.0, expiry, ts, 0.5, 0.6, 10, 1, 7.25, "regular"),
            (1045.0, "C", 900.0, 0.60, 0.09, 900.0, expiry, ts, 0.5, 0.5, 10, 1, 7.25, "regular"),  # 압도적 1등
            (1050.0, "P", 20.0, 0.62, 0.02, -200.0, expiry, ts, 0.5, -0.4, 10, 1, 7.25, "regular"),
        ],
    }
