...
def test_build_expiry_liquidity_table_formats_pct_spread_not_dollar():
    ...
    assert list(labels) == ["먼슬리", "위클리(월)", "위클리(목)"]
    assert list(expiries) == ["2026-07-30", "2026-07-13", "2026-07-16"]
    assert list(spreads) == ["4.10%", "9.30%", "10.10%"]  # Cao-Wei %스프레드 — 달러 스프레드 아님
    assert list(depths) == ["220", "70", "65"]
    assert list(volumes) == ["480", "140", "130"]
    assert list(days) == ["24", "3", "6"]
...
def test_build_expiry_liquidity_table_notes_monthly_expiry_week():
    ...
    # 위클리(목) 신규 상장이 없는 경우(2026-07-10 실측 근거)를 사람이 바로 알아볼 수 있어야 한다.
    # 위클리(월)은 요일이 달라 영향받지 않으므로 이번 주(7/13) 그대로 둔다.
    rows = [
        {
            "series": "regular",
        ...
        {
            "series": "weekly_thu",
            "expiry": date(2026, 7, 16),  # 이번 주가 아니라 차주 위클리(목)
            "atm_spread_pct": 0.05,
            "depth": 90.0,
            "volume": 200.0,
            "days_to_expiry": 7,
